...
describe("InteractiveField", () => {
    ...
    it("renders passed component with field/onClick params and proper field size", () => {
        const field = shallow(<InteractiveField fieldComponent={FakeComponent} xSize={1} ySize={2} playerMarks={["x"]} />).find(FakeComponent);
    ...
    it("updates field on calling onClick with correct values", () => {
        const wrapper = shallow(<InteractiveField fieldComponent={FakeComponent} xSize={2} ySize={2} playerMarks={["x"]} />);
    ...
    it("changes player mark on eash move", () => {
        const wrapper = shallow(<InteractiveField fieldComponent={FakeComponent} xSize={8} ySize={1} playerMarks={["x", "y", "z"]} />);
        expect(wrapper.find(FakeComponent).props()).toEqual({
            field: [["", "", "", "", "", "", "", ""]],
            onClick: expect.any(Function),
        });
    ...
    it("updates field on calling onClick ignores incorrect clicks", () => {
        const wrapper = shallow(<InteractiveField fieldComponent={FakeComponent} xSize={2} ySize={2} playerMarks={["x", "y"]} />);
